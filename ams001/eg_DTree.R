...
model <- rpart(output ~ age + sex + exng + caa + cp + trtbps + chol + fbs + restecg +
                 thalachh + slp + oldpeak + thall, data = train_data, method = "class")

# Visualize the decision tree
rpart.plot(model)

# Model Evaluation
predicted_output <- predict(model, newdata = test_data, type = "class")
